...
object Util {
  ...
  def getDateTimeFromUTCTimestamp(tsValue: String): DateTime = ISODateTimeFormat.dateTimeParser().parseDateTime(tsValue)

  def getNoOfOfPartitions(minTimestamp: DateTime, maxTimestamp: DateTime): Int = {
    /*
    get number of days
    get number of hours
    get number of minutes
    24 is the parts for days
    6 is the parts for the hour
    based on above data apply config setting value to determine the number of tasks
     */
    val DAY_LEVEL_PARTITIONS = 24
    val HOUR_LEVEL_PARTITIONS = 6
    val daysDuration = Days.daysBetween(minTimestamp, maxTimestamp).getDays
    val hourDuration = Hours.hoursBetween(minTimestamp, maxTimestamp).getHours
    daysDuration * DAY_LEVEL_PARTITIONS * HOUR_LEVEL_PARTITIONS + hourDuration - (DAY_LEVEL_PARTITIONS * daysDuration) * HOUR_LEVEL_PARTITIONS
  }


  def getTimestampPredicates( mysqlDBConf: DBConfiguration,sqlContext: SQLContext, internalConfig: InternalConfig,
                              distKey:String, minTimestamp: DateTime, maxTimestamp: DateTime): Option[Seq[String]] = {
    val beginEpoch = minTimestamp.getMillis
    val endEpoch = maxTimestamp.getMillis
    val noOfPartitions = getNoOfOfPartitions(minTimestamp, maxTimestamp)
    val delta = (maxTimestamp.getMillis - minTimestamp.getMillis + 1) / noOfPartitions
    val predicates: Seq[String] = (1 to noOfPartitions).map{
      n => {
        val startTimestamp = new Instant(beginEpoch+(n-1)*delta).toDateTime
        val endTimestamp = new Instant(beginEpoch+n*delta).toDateTime
        s"$distKey BETWEEN $startTimestamp AND $endTimestamp "
      }
    }
    val startTimestamp = new Instant(beginEpoch+noOfPartitions*delta).toDateTime
    val endTimestamp = new Instant(endEpoch).toDateTime
    Some(predicates :+ s"$distKey BETWEEN $startTimestamp AND $endTimestamp".toString)

  }

  def getIntegralPredicates(mysqlConfig: DBConfiguration, sqlContext: SQLContext, internalConfig: InternalConfig, distKey: String): Option[Seq[String]] = {
    ...
    //         None
    // }
    val minMaxTmp: (String, String) = Util.getMinMax(mysqlConfig, distKey)
    ...
    if (mapPartitions == 0) {
      ...
      val predicates = (0 to mapPartitions).
        map { n =>
